...
console.log('Diagnostic: Lecture de la base de donnees...');
...
try {
  ...
  if (usersRow) {
    try {
      ...
      console.log('[OK] Cle "users": ' + (Array.isArray(users) ? users.length : 0) + ' comptes');
    } catch (e) {
      console.log('[ERR] Erreur parsing users');
    }
  }
  ...
  if (gcUsersRow) {
    try {
      ...
      console.log('[OK] Cle "gc-users": ' + (Array.isArray(gcUsers) ? gcUsers.length : 0) + ' comptes');
    } catch (e) {
      console.log('[ERR] Erreur parsing gc-users');
    }
  }
  ...
  if (source.length === 0) {
    console.log('[ERR] Aucun compte en base !');
    process.exit(1);
  }
...
<head>
  ...
  <title>Synchronisation Utilisateurs - SI Genie Consultant</title>
  <style>
...
<body>
  <div class="container">
    ...
    <div class="status">
      ...
      <div class="status-item">
        <span class="status-label">Base de donnees:</span>
        <span class="status-value">si_genie.db</span>
      </div>
      <div class="status-item">
        <span class="status-label">Etat LocalStorage:</span>
        <span class="status-value" id="localStorageStatus">Non synchronise</span>
      </div>
    </div>

    <h2 style="font-size: 16px; margin-bottom: 10px; color: #A0B0C8;">Comptes a synchroniser:</h2>
...
${usersList}
  ...
  <script>
    ...
    function checkStatus() {
      try {
        ...
        if (!stored) {
          showInfo('Aucune donnee utilisateur dans localStorage');
          return;
        }
    ...
    setTimeout(() => {
      try {
        ...
        if (stored) {
          const parsed = JSON.parse(stored);
          document.getElementById('localStorageStatus').textContent = (Array.isArray(parsed) ? parsed.length : 0) + ' comptes synchronises';
        }
      } catch (_) {}
    }, 100);
  </script>
  ...
  console.log('[OK] Page de synchronisation generee');
  ...
  console.log('4. Connexion avec les comptes synchronises');
