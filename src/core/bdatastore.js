...
const SAVED_PROXY_URL = (() => {
  try {
    ...
    // ⚠️ CRITICAL FIX v10.5.9: Ignore localhost URLs from localStorage if accessing from remote IP
    // If frontend is accessed from a non-localhost IP (e.g., 192.168.1.133),
    // don't use a saved localhost:3001 URL — use the current hostname instead
    const currentHostname = typeof window !== 'undefined' ? window.location.hostname : null;
    const isRemoteAccess = currentHostname && !currentHostname.match(/^(localhost|127\.0\.0\.1|::1)$/);
    const savedIsLocalhost = saved.match(/localhost|127\.0\.0\.1|::1/);
    
    if (isRemoteAccess && savedIsLocalhost) {
      console.warn('⚠️ Ignoring localhost URL from localStorage on remote access:', saved);
      return null;
    }
...
export const SHARED_KEYS = new Set([
  ...
  // [FIX P0-01 / C-04] config IA partagée (moteur par défaut, activation, libellés —
  // JAMAIS les clés API, toujours vidées avant dsSave, voir gcAISaveConfig dans helpers.js)
  'gc-ai-config-v1',
  ...
  // ── FIX v154 — Clés utilisées par dsSave/dsGet/useRemoteSync mais absentes de
  // SHARED_KEYS : ces clés ne quittaient JAMAIS le localStorage local (isSharedKey
  // les rejetait avant même la tentative réseau), d'où la désynchronisation totale
  // et silencieuse du Plan Comptable OHADA / État Financier entre postes, malgré un
  // dsSave() qui retournait { ok:true } (succès trompeur — écriture locale seulement).
  'gc-ohada-active-plan',         // plan comptable actif (importé/personnalisé) — lu par État Financier
  'gc-ohada-active-plan-version', // ID de la version de plan comptable actuellement active
  'gc-ohada-plan-versions',       // historique des versions de plan comptable importées
  'gc-ohada-docs',                // documents téléversés OHADA / finance
  'gc-factory-reset-signal',      // signal de réinitialisation usine cross-machine
  'gc-rapport-generated',         // rapports d'activité générés
  'gc-rapport-requests',          // demandes de rapport d'activité
]);
...
function _loadTombstones() {
  try {
    const parsed = JSON.parse(_lsGet(TOMBSTONE_LS_KEY) || '{}');
    // [FIX 2026-06-22] 'gc-tombstones' doit être un objet { clé: [ids] }.
    // S'il a été corrompu en tableau (observé en prod), on repart de {} plutôt
    // que de propager une forme invalide (qui ferait perdre silencieusement
    // toute future écriture tombstones[key]=[...] au JSON.stringify).
...
export async function dsDeleteItemFromArray(listKey, itemId, userId = null) {
  ...
  // 1. Enregistrer tombstone IMMÉDIATEMENT (local + queue pour flush si offline)
  ...
  // 3. Mettre à jour localement (optimistic update)
  const newList = current.filter(item => item?.id && String(item.id) !== String(itemId));
  lsSave(listKey, newList);
  _cache.set(listKey, { data: newList, ts: Date.now() });
  try { _lsSet('__ts__:' + listKey, String(Date.now())); } catch {}

  // 4. [T-06 FIX] Le DELETE endpoint côté serveur gère atomiquement : tombstone serveur +
  //    filtre dans le tableau + broadcast data_changed. On NE FAIT PLUS de dsSave(forceOverwrite)
  //    après : forceOverwrite sautait le merge protégé, créant un risque d'écrasement du travail
  //    concurrent. Si le serveur est offline, le tombstone local suffira au flush hors-ligne.
  if (_online) {
    try {
      const r = await fetch(
      ...
      if (r.ok) return { ok: true, synced: true };
      // Le DELETE a échoué (réseau, DB) → tomber en repli : push du nouveau tableau filtré
      console.warn(`[dsDeleteItemFromArray] DELETE HTTP ${r.status} pour ${listKey}/${itemId} → repli dsSave`);
      return dsSave(listKey, newList, userId);
    } catch (_e) {
      // Offline ou timeout : enqueue le nouveau tableau pour le prochain flush
      enqueueOffline(listKey, newList, userId);
...
// [FIX RACINE 2026-06-22] dequeueOfflineKey — retire de la file hors-ligne toute
// entrée en attente pour `key`. À appeler après un dsSave() réussi EN LIGNE pour
// cette clé : la valeur vient d'être confirmée par le serveur, donc tout instantané
// plus ancien encore en file (mis en attente lors d'une précédente micro-coupure)
// est désormais obsolète. Sans ce nettoyage, cet instantané périmé restait en file
// et était renvoyé tel quel au prochain flush (reconnexion WebSocket), écrasant la
// donnée fraîche qu'on vient justement de confirmer — c'est ce mécanisme qui a fait
// disparaître le client "MINDZIE NGOMO OPINA" et le dossier "DOS-A02-O02.02/2026".
// Le serveur est désormais lui aussi protégé (applyProtectedWrite côté api-proxy.js)
// mais ce nettoyage côté client supprime le problème à la source.
function dequeueOfflineKey(key) {
  try {
    const q = loadOfflineQueue();
    const filtered = q.filter(item => item.key !== key);
    if (filtered.length !== q.length) saveOfflineQueue(filtered);
  } catch (_) {}
}
...
async function flushOfflineQueue() {
  ...
  try {
    ...
    // [FIX P1-05] Pousser les tombstones/wipe-registry EN PREMIER dans le batch : si un autre
    // poste a supprimé un item pendant que ce poste était hors-ligne, le serveur doit connaître
    // ce tombstone AVANT de recevoir les données métier de ce même batch, sinon l'anti-régression
    // serveur ne peut pas filtrer l'item supprimé et celui-ci peut réapparaître chez tous.
    batch.sort((a, b) => {
      const aPri = (a.key === 'gc-tombstones' || a.key === 'gc-wipe-registry') ? -1 : 0;
      const bPri = (b.key === 'gc-tombstones' || b.key === 'gc-wipe-registry') ? -1 : 0;
      return aPri - bPri;
    });
    ...
    for (const item of batch) {
      try {
        const r = await fetch(`${getProxyUrl()}/api/data/${encodeURIComponent(item.key)}`, {
          method:  'POST',
          // [FIX P1-04] x-offline-queued-at : timestamp de mise en file (pas d'envoi). Permet au
          // serveur de comparer avec son updated_at et de rejeter une entrée antérieure à la
          // dernière écriture connue — sans ce header, un poste resté longtemps hors-ligne pouvait
          // écraser au reconnect des données plus récentes poussées par d'autres postes pendant son absence.
          headers: getRequestHeaders({ 'x-offline-queued-at': String(item.ts || 0) }),
        ...
        if (!r.ok) {
          ...
          if (r.status === 429 || r.status >= 500) {
            // FIX v154 — ne pas spread directement le résultat de batch?.slice(...) :
            // si batch est undefined, ?. renvoie undefined et "...undefined" lève une
            // TypeError ("undefined is not iterable"), ce qui déplaçait le crash au lieu
            // de le corriger (détecté par eslint no-unsafe-optional-chaining).
...
async function initWebSocket() {
  try {
    ...
    // [FIX P2-06] usePurifySync.js s'abonne à window._gcSocket pour recevoir 'gc-purify-order'
    // (et émettre 'gc-client-validate'). Sans cette exposition, jamais assignée nulle part dans
    // le projet, ces événements socket.io n'atteignaient jamais le hook côté client.
    try { if (typeof window !== 'undefined') window._gcSocket = _socket; } catch (_) {}
    ...
    _socket.on('sync_init', ({ ts, reason, tombstones, wipeRegistry } = {}) => {
      ...
      // [FIX P1-07] Fusionner les tombstones/wipeRegistry serveur reçus à la reconnexion — sans
      // cela, ce client ne connaissait les suppressions des autres postes que via le broadcast
      // 'data_changed' en temps réel (manqué pendant la déconnexion), jamais via un catch-up explicite.
      if (tombstones && typeof tombstones === 'object') {
        try {
          const local = _loadTombstones();
          const merged = {};
          for (const key of new Set([...Object.keys(local), ...Object.keys(tombstones)])) {
            merged[key] = [...new Set([...(local[key] || []), ...(tombstones[key] || [])])]?.slice(-5000);
          }
          _lsSet(TOMBSTONE_LS_KEY, JSON.stringify(merged));
        } catch {}
      }
      if (wipeRegistry && typeof wipeRegistry === 'object') {
        try { _updateWipeRegistry(wipeRegistry); } catch {}
      }
      _syncListeners.forEach(fn => {
...
export async function dsSave(key, value, userId = null, options = {}) {
  // [T-05] Injection automatique de updatedAt sur les items sans horodatage.
  // Sans ceci, le last-writer-wins de applyProtectedWrite (serveur) compare toujours
  // 0 >= 0 → l'entrant gagne toujours, même s'il est périmé — cause directe des
  // modifications qui disparaissent et des données qui régressent au flush offline.
  // Seuls les items sans updatedAt existant sont touchés : on ne modifie jamais
  // le timestamp d'un item déjà horodaté (cela casserait le LWW dans l'autre sens).
  let value_with_ts = value;
  if (Array.isArray(value) && value.length > 0 && isSharedKey(key)) {
    const now = new Date().toISOString();
    let patched = false;
    value_with_ts = value.map(item => {
      if (!item || typeof item !== 'object' || item.updatedAt) return item;
      patched = true;
      return { ...item, updatedAt: now };
    });
    if (!patched) value_with_ts = value; // aucun item modifié → réutiliser la ref d'origine
  }
  const finalValue = value_with_ts; // alias sans redéclaration de 'value' (paramètre de la fonction)

  lsSave(key, finalValue);
  const _writeNow = Date.now();
  _cache.set(key, { data: finalValue, ts: _writeNow });
  ...
  if (!_online) {
    enqueueOffline(key, finalValue, userId); // [D5] validation dans enqueueOffline
    startOfflineRetryLoop();
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  let sendValue = finalValue;
  if (shouldProtectAgainstRegression(key, finalValue) && Array.isArray(finalValue) && !options.forceOverwrite) {
    ...
    // [FIX P1-03] Seuil abaissé à 25% (régression vraiment anormale)
    // Ancien seuil : 50% → une suppression en lot de 60% (nettoyage volontaire fréquent,
    // ex: purger les tâches terminées) déclenchait systématiquement le merge et restaurait
    // les éléments supprimés. Nouveau seuil : seule une suppression de 75%+ du contenu
    // (anormale dans l'usage normal) déclenche la protection défensive.
    // FIX BUG#2 — Ne pas merger si forceOverwrite est set
    if (!options.forceOverwrite && Array.isArray(cachedData) && cachedData.length >= 2 && finalValue.length <= cachedData.length * 0.25) {
      ...
      if (preserved.length > 0) {
        ...
        const merged = [...finalValue, ...preserved];
  ...
  try {
    ...
    // [FIX RACINE 2026-06-22] Cette sauvegarde vient de réussir EN LIGNE : tout
    // instantané plus ancien encore en file d'attente hors-ligne pour cette même
    // clé est désormais obsolète et ne doit plus être renvoyé au prochain flush.
    dequeueOfflineKey(key);
