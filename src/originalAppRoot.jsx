...
// FIX v127 — Import unique depuis core/index.js (suppression du doublon ligne 19)
import {
  _lsGet, _lsSet, _lsRm, lsLoad, lsSave, lsLoadSecure,
  _noop, _tDone, _tActive,
  gcPushNotif, playSound, formatCFA, gcCalcPaie, gcCalcIRPP, gcLoadFiscalConfig,
  gcFindApprover, gcCodif, gcCodifDOC, gcCodifTCHE, gcCodifMSG, _gcSafeCalc,
  formatDate, gcGetDelaiConfig, gcAntiRedondance,
  gcAIBuildSystemPrompt, gcAILoadConfig, gcAISaveConfig, GC_AI_PROXY_URL, _gcProxyFetch,
  gcCopy, generateAccessCode, _dataUrlToBlob,
  _gcEncrypt, _gcDecrypt, _lsGetSecure, _lsSetSecure,
  useSI, SICtx, getUserProcess, _migrateDGAccount, _checkSchemaVersion, _migrateLS, SIErrorBoundary,
  gcMigrateFilesFromLS, gcSyncFilesToServer, gcFileStats, dsInitSync, dsStartSync, dsOnSync, dsSave, dsLoad, dsGet, dsSet, SHARED_KEYS,
  // FIX vSERVER-TIME — synchronisation heure serveur
  syncServerTime,
  // FIX v127 — fonctions IP centralisées (étaient dans HubPanels, causaient ReferenceError ici)
  gcGetClientIp, _gcCachedIp,
  // FIX v127 — fonctions session/hash (étaient non exportées dans constants.js)
  gcHashPassword, gcGenerateSessionToken, gcValidateSessionToken, gcVerifyPassword,
} from './core/index.js';
import {
  THEMES, STATUS_CONFIG, PRIORITY_CONFIG,
  INITIAL_DOSSIERS, INITIAL_TACHES, INITIAL_RDVS, INITIAL_PENDING, INITIAL_PARTNERS,
  INITIAL_ARCHIVES, INITIAL_MESSAGES, INITIAL_USERS, INITIAL_COMMITTEES,
  INITIAL_CODIF_REGISTRY, INITIAL_INTERNAL_DOCS, INITIAL_SI_SYSTEM_DOCS,
  // FIX v127 — manquaient dans les imports originaux
  INITIAL_SESSION_LOGS, INITIAL_ACCOUNT_ACTIONS, ACCOUNT_STATUS_CONFIG,
  GC_SUBPROC_MAP, GC_ALL_SUBPROCS, GC_APPROVAL_ROUTING, GC_CIRCUITS_INIT,
  GC_DOCS_REQUIS, GC_ACTIVITES, GC_DELAI_DEFAULT,
  GC_AI_CONFIG_KEY, GC_AI_CONFIG_DEFAULT, GC_AI_LEVEL_RULES,
  PROCESS_ACTIVITIES, DOC_CATEGORIES, USER_FUNCTIONS, PROCESS_APP_MATRIX_DEFAULT,
  CRM_SEGMENTS_C, CRM_SECTEURS_C, CRM_SOURCES_C, CRM_TYPES_INTERACTION_C,
  CRM_TYPES_RELANCE_C, CRM_ETAPES_C, CRM_RISKS_C, CRM_KYC_C, CRM_STATUTS_C,
  CRM_PROCS_METIER_C, PLAN_COMPTABLE_OHADA, GC_ADMIN_HASH, GC_DG_HASH,
  ALL_NATIONALITIES, DEMO_USERS, DEMO_DOSSIERS, DEMO_RDVS, DEMO_TACHES,
  DEMO_PENDING, DEMO_ARCHIVES, DEMO_CODIF_REGISTRY, DEMO_INTERNAL_DOCS,
  FILE_TYPE_CONFIG, INITIAL_SIRH_PRESENCES, INITIAL_SIRH_LEAVES, INITIAL_RECRUTEMENTS,
} from './core/constants.js';
import { Btn, Modal, InputField, SelectField, PrintButton, QRDisplay, Tabs, NationaliteField, SmartBanner, RotatingAlert } from './components/UI.jsx';
import { GlobalStyles } from './styles/GlobalStyles.jsx';
import { CoverPage, LoginPage, CreateAccountPage, NotificationCenter } from './components/Auth.jsx';
...
export default function App() {
  ...
  const setSiSystemDocs = useCallback((v) => {
    setSiSystemDocsState(prev => {
      ...
      try { _lsSet("gc-si-docs", JSON.stringify(resolved.map(d => ({ ...d })))); } catch (e) {
        try {
  ...
  const setRequireConnApproval = useCallback((v) => {
    ...
    try { _lsSet("gc-require-conn-approval", JSON.stringify(v)); } catch (_) {}
  }, []);
  ...
  const setPendingConnections = useCallback((updater) => {
    setPendingConnectionsState(prev => {
      ...
      try { _lsSet("gc-pending-connections", JSON.stringify(resolved)); } catch (_) {}
      return resolved;
    });
  }, []);
...
// INTENTIONNEL : addSessionLog est un useCallback stable, setSessionLogs ne doit pas re-créer la fonction
  ...
  const setPAA = useCallback((v) => {
    setPendingAccountActions(prev => {
      const resolved = typeof v === 'function' ? v(prev) : v;
      try { _lsSet("gc-account-actions", JSON.stringify(resolved)); } catch (_) {}
  ...
  useEffect(() => {
    ...
    dsInitSync().then(result => {
      if (result.online) {
        ...
        // FIX v136 — Timeout de sécurité : si le serveur tarde > 3s, débloquer le login
        const hydrateTimeout = setTimeout(() => {
          console.warn('[SI] ⏱️ Timeout hydratation (3s) — login débloqué avec données locales');
          setHydrating(false);
        }, 3000);

        Promise.all(HYDRATE_MAP.map(async ({ key, setters, fallback }) => {
          try {
          ...
          clearTimeout(hydrateTimeout); // FIX v136 — annuler le timeout si l'hydratation est à l'heure
          setHydrating(false);          // FIX v136 — débloquer le login maintenant que users est prêt
          console.log('[SI] ✅ Hydratation initiale terminée — SI prêt, aucune donnée écrasée');
        });
  ...
  useEffect(() => {
    const unsub = dsOnSync((event) => {
      ...
      try {
        switch(stateKey) {
          case 'users':            setUsersState(dsLoad("users", INITIAL_USERS)); setProdUsers(dsLoad("users", INITIAL_USERS)); break;
          case 'dossiers':         setDossiersState(dsLoad("dossiers", INITIAL_DOSSIERS)); setProdDossiers(dsLoad("dossiers", INITIAL_DOSSIERS)); break;
          case 'taches':           setTachesState(dsLoad("taches", INITIAL_TACHES)); setProdTaches(dsLoad("taches", INITIAL_TACHES)); break;
          case 'rdvs':             setRdvsState(dsLoad("rdvs", INITIAL_RDVS)); setProdRdvs(dsLoad("rdvs", INITIAL_RDVS)); break;
          case 'pendingApprovals': setPendingApprovalsState(dsLoad("pendingApprovals", INITIAL_PENDING)); setProdPending(dsLoad("pendingApprovals", INITIAL_PENDING)); break;
          case 'partners':         setPartnersStateRaw(dsLoad("partners", INITIAL_PARTNERS)); setProdPartners(dsLoad("partners", INITIAL_PARTNERS)); break;
          case 'siAppearance':     setSiAppearanceState(dsLoad("siAppearance", null)); break;
          case 'siLogoUrl':        setSiLogoUrlState(dsLoad("siLogoUrl", null)); break;
          case 'siCSSOverrides':   setSiCSSOverridesState(dsLoad("siCSSOverrides", null)); break;
          case 'gc-session-logs':          setSessionLogs(dsLoad("gc-session-logs", [])); break;
          case 'gc-pending-connections':   setPendingConnectionsState(dsLoad("gc-pending-connections", [])); break;
          case 'gc-require-conn-approval': setRequireConnApprovalState(dsLoad("gc-require-conn-approval", false)); break;
          case 'standaloneDocuments': {
            // Déclencher un événement personnalisé pour que SIApp mette à jour son état
          ...
          case 'gc-account-actions': {
            dsGet("gc-account-actions", null).then(val => {
              if (val !== null) {
...
// FIX v140 — Re-exécuté quand hydrating passe à false (données serveur disponibles)
  }, [hydrating]); // eslint-disable-line react-hooks/exhaustive-deps
...
// INTENTIONNEL : surveiller les tâches uniquement
  ...
  const handleFactoryReset = async () => {
    ...
    // -- SIRH --
    try { _lsSet("gc-sirh-presences", "[]"); } catch (_) {}
    try { _lsSet("gc-sirh-leaves", "[]"); } catch (_) {}
    try { _lsSet("gc-sirh-recrutements", "[]"); } catch (_) {}
    try { _lsSet("gc-sirh-evaluations", "[]"); } catch (_) {}
    try { _lsSet("gc-paie-transferts", "[]"); } catch (_) {}
    // -- Documents --
    try { _lsSet("gc-internal-docs", "[]"); } catch (_) {}
    try { _lsSet("gc-external-docs", "[]"); } catch (_) {}
    try { _lsSet("gc-dossier-files", "[]"); } catch (_) {}
    // -- Messagerie --
    try { _lsSet("gc-messages-global", "[]"); } catch (_) {}
    try { _lsSet("gc-courrier-docs", "[]"); } catch (_) {}
    // -- Journaux --
    try { _lsSet("gc-session-logs", "[]"); } catch (_) {}
    try { _lsSet("gc-account-actions", "[]"); } catch (_) {}
    ...
    try { _lsSet("gc-codif-registry", "[]"); } catch (_) {}
    // -- Productivité --
