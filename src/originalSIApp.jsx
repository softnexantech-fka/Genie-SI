...
import { OHADARefApp, DelaiConfigPanelO01, FacturationModule, ConventionModule, PaieSimulateur, FiscalConfigPanel, ExportBackupPanel } from './modules/finance/FinanceApp.jsx';
  ...
  useEffect(() => {
    ...
    const pushPresence = async () => {
      ...
      try {
        ...
        // Purger les entrées > 60s avant de sauvegarder
        const purged = {};
        Object.entries(existing).forEach(([uid, ts]) => {
          if (now - ts < 60000) purged[uid] = ts;
        });
...
// INTENTIONNEL : currentUser.level lu via Notification.permission guard (no stale risk)
  ...
  const [msgUnreadCount, setMsgUnreadCount] = useState(() => {
    try {
      ...
      if (saved) {
        ...
        return msgs.filter(m => !m.read && m.type !== "group" && m.from !== uid && (m.to?.includes(uid) || m.to?.includes("ALL"))).length;
      }
    } catch (_) {}
    return 0;
  });
      ...
      try {
        ...
        if (saved) {
          ...
          const cnt = msgs.filter(m =>
            !m.read && m.type !== "group" && m.from !== uid &&
            (m.to?.includes(uid) || m.to?.includes("ALL"))
          ).length;
          setMsgUnreadCount(cnt);
        }
      } catch (_) {}
    };
  ...
  const [codifRegistry, setCodifRegistry] = useState(INITIAL_CODIF_REGISTRY);
            ...
            _dsGet('gc-presence', {}).then(val => {
              if (val && typeof val === 'object') {
                ...
                Object.entries(val).forEach(([uid, ts]) => {
                  if (now - ts < 45000) window.__gcOnlineUsers.set(uid, ts);
          ...
          import('./core/datastore.js').then(({ dsGet }) => {
            dsGet('gc-messages-global', []).then(val => {
              if (val && Array.isArray(val)) {
                ...
                // Recalculer le badge non-lu pour l'utilisateur courant
                const uid = localUser?.id;
                if (uid) {
                  const unread = val.filter(m =>
                    !m.read &&
                    m.from !== uid &&
                    (m.to?.includes(uid) || m.to?.includes('ALL') ||
                     (m.type === 'group' && m.groupMembers?.includes(uid)))
                  ).length;
  ...
  const dispatchSyncEvent = useCallback((type, payload) => {
    ...
    switch(type) {
      ...
      case "RECRUTEMENT_VALIDATED": {
        ...
        setTaches(prev => [...prev, onboardingTask]);
        setNotifications(prev => [{
          id: "N"+Date.now(), icon: "🎯",
...
// INTENTIONNEL : snapshot dossiers/users au montage puis polling
  ...
  const saveDossierFiles = useCallback((v) => {
    setDossierFiles(prev => {
      ...
      if (!isDemoMode) {
        ...
        dsSave('gc-dossier-files', resolved).catch(() => {});
      }
      return resolved;
    });
  }, [isDemoMode]);
  ...
  const saveStandaloneDocs = useCallback((v) => {
    setStandaloneDocumentsRaw(prev => {
      const resolved = typeof v === "function" ? v(prev) : v;
      if (!isDemoMode) {
        // FIX v63 C7  -  IndexedDB pour les documents standalone
        // FIX v129 — Aussi sync SQLite pour les autres postes réseau.
        gcIDB.putAll(gcIDB.STORE_DOCS, resolved).catch(() => {});
        dsSave('gc-standalone-docs', resolved).catch(() => {});
        // S'assurer que la clé locale est aussi synchronisée si elle est utilisée par d'autres composants
        dsSave('standaloneDocuments', resolved).catch(() => {});
      }
      return resolved;
    });
  }, [isDemoMode]);
...
// FIX v72 — FILE_TYPE_CONFIG et getFileType déplacés au scope module (voir définition avant FileDataManager)
  ...
  const renderModule = () => {
    switch (activeModule) {
      ...
      case "bureau":          return <BureauOffice key="bureau" T={T} currentUser={localUser} setNotifications={setNotifications} rdvs={rdvs} dossiers={dossiers} setDossiers={setDossiers} partners={partners} setPartnersSync={setPartnersSync} appHabilitations={appHabilitations} appAccessCodes={appAccessCodes} users={users} setUsers={setUsers} taches={taches} setTaches={setTaches} setRdvs={setRdvs} isDemoMode={isDemoMode} pendingApprovals={pendingApprovals} setPendingApprovals={setPendingApprovals} />;
        ...
        <ProfilePage
          ...
          onSave={(updatedUser) => {
            ...
            setUsers(prev => {
              ...
              try { _lsSet("gc-users", JSON.stringify(n)); } catch (_) {}
              return n;
            });
