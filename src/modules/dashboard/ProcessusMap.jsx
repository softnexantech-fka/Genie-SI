...
import { _lsGet, _lsSet, playSound, formatDate, generateAccessCode, useSI, getUserProcess, gcFileSave, _activeUser, gcViewDoc, getProcColor, gcReadFile, gcFmtSize, gcDownloadDoc, gcDocIcon, ALPHA_SEQ, dsSave } from '../../core/index.js';
...
export const ProcessusMap = React.memo(function ProcessusMap() {
  ...
  // FIX v152 — vrais objets File en attente d'upload via gcFileSave
  const [pendingIntFile, setPendingIntFile] = useState(null);
  const [pendingExtFile, setPendingExtFile] = useState(null);
  ...
  const handleIntDocFileSelect = async (e) => {
    ...
    // FIX v152 — conserver le vrai objet File pour upload serveur
    setPendingIntFile(file);
    try {
      ...
      setDocUploadStatus({ type: "ok", msg: `✅ ${result.name} (${result.sizeStr}) — sera téléversé au serveur` });
    } catch (err) {
  ...
  // FIX v152 — handleUploadDoc async : upload réel via gcFileSave (serveur ou IDB)
  const handleUploadDoc = async () => {
    if (!docUploadForm.name) { gcAlert("Veuillez saisir le nom du document."); return; }
    // Upload du fichier si présent (pendingIntFile stocké dans le state)
    let serverRef = null;
    if (pendingIntFile) {
      try {
        serverRef = await gcFileSave(pendingIntFile, {
          module: 'processus-internal',
          uploadedBy: localUser.id,
          nom: pendingIntFile.name,
          taille: pendingIntFile.size,
          type: pendingIntFile.type,
        });
      } catch (err) {
        console.warn('[ProcessusMap] Upload fichier interne :', err.message);
      }
    }
    const newDoc = {
      id: "IDOC-" + Date.now(),
      ...
      fileSize: docUploadForm.fileSize || serverRef?.size || 0,
      sizeStr: gcFmtSize(docUploadForm.fileSize || serverRef?.size || 0),
      hasFile: !!(serverRef || docUploadForm.dataUrl),
      // FIX v152 — priorité serverUrl, fallback dataUrl
      dataUrl: serverRef?.serverUrl ? null : (docUploadForm.dataUrl || null),
      serverUrl: serverRef?.serverUrl || null,
      serverId: serverRef?.serverId || null,
      storageType: serverRef?.serverUrl ? 'server' : 'local',
      fileName: serverRef?.nom || docUploadForm.fileName || null,
  ...
  const handleExtDocFileSelect = async (e) => {
    ...
    // FIX v152 — conserver le vrai objet File pour upload serveur
    setPendingExtFile(file);
    try {
      const result = await gcReadFile(file, 10);
      setExtDocForm(f => ({ ...f, dataUrl: result.dataUrl, fileName: result.name, fileSize: result.size, fileType: result.ext }));
      setExtDocStatus({ type: "ok", msg: `✅ ${result.name} (${result.sizeStr}) — sera téléversé au serveur` });
    } catch (err) {
      setPendingExtFile(null);
      setExtDocStatus({ type: "error", msg: `❌ ${err.message}` });
    }
    if (e.target) e.target.value = "";
  };

  // FIX v152 — handleSaveExtDoc async : upload réel via gcFileSave
  const handleSaveExtDoc = async () => {
    if (!extDocForm.name.trim()) { gcAlert("Veuillez saisir le nom du document."); return; }
    let serverRef = null;
    if (pendingExtFile) {
      try {
        serverRef = await gcFileSave(pendingExtFile, {
          module: 'processus-external',
          uploadedBy: localUser.id,
          nom: pendingExtFile.name,
          taille: pendingExtFile.size,
          type: pendingExtFile.type,
        });
      } catch (err) {
        console.warn('[ProcessusMap] Upload fichier externe :', err.message);
      }
    }
    const newDoc = {
      id: "EDOC-" + Date.now(),
      ...
      fileSize: extDocForm.fileSize || serverRef?.size || 0,
      sizeStr: gcFmtSize(extDocForm.fileSize || serverRef?.size || 0),
      // FIX v152 — stocker serverUrl, strip du dataUrl si serverUrl disponible
      dataUrl: serverRef?.serverUrl ? null : (extDocForm.dataUrl || null),
      serverUrl: serverRef?.serverUrl || null,
      serverId: serverRef?.serverId || null,
      storageType: serverRef?.serverUrl ? 'server' : 'local',
      fileName: serverRef?.nom || extDocForm.fileName || null,
  ...
  // FIX v152 — priorité serverUrl dans les téléchargements
  const handleDownloadIntDoc = (doc) => {
    if (!doc.dataUrl && !doc.serverUrl && !doc.serverId) { gcAlert("⚠️ Aucun fichier disponible. Le document n'a pas encore de fichier téléversé."); return; }
    setInternalDocs(prev => prev.map(d => d.id === doc.id ? { ...d, downloads: (d.downloads||0)+1 } : d));
    gcDownloadDoc(doc);
  };

  const handleDownloadExtDoc = (doc) => {
    if (!doc.dataUrl && !doc.serverUrl && !doc.serverId) { gcAlert("⚠️ Aucun fichier disponible pour ce document externe."); return; }
  ...
  const handleSaveCommittee = () => {
    ...
    setCommittees(prev => {
      const updated = prev.map(c => c.id === committeeEditForm.id ? committeeEditForm : c);
      // FIX v142 — Persister les modifications de comité sur le serveur (SQLite cross-machine)
      dsSave('gc-committees', updated).catch(() => {});
      return updated;
    });
                            ...
                            {isEditing && procEditForm ? (
                              ...
                              <div>
                                ...
                                <div style={{ display:"flex",gap:8 }}>
                                  <button onClick={()=>{
                                    // FIX v154 CRITICAL — setProcessConfig seul ne persiste PAS.
                                    // Après refresh, processConfig est rechargé depuis les constants initiales.
                                    // On doit sauvegarder dans dsSave ET localStorage pour survivre au refresh.
                                    const newConfig = {...processConfig, [code]: {...(processConfig[code]||{}), ...procEditForm}};
                                    setProcessConfig(newConfig);
                                    // Persistance localStorage (immédiate, synchrone)
                                    try { _lsSet('gc-process-config', JSON.stringify(newConfig)); } catch(_) {}
                                    // Persistance serveur/datastore (async, pour sync inter-machines)
                                    dsSave('gc-process-config', newConfig).catch(() => {});
        ...
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {visibleInternalDocs.map(doc => {
              ...
              return (
                <div key={doc.id} style={{ background:T.surface2, borderRadius:10, border:`1px solid ${(doc.dataUrl||doc.serverUrl)?"#22C55E33":T.border}`, padding:"12px 16px", display:"flex", gap:12, alignItems:"flex-start" }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:"flex", gap:6, marginTop:6, flexWrap:"wrap", alignItems:"center" }}>
                      ...
                      {(doc.dataUrl||doc.serverUrl)
                        ? <Badge label={`✅ ${doc.sizeStr || gcFmtSize(doc.fileSize)}${doc.storageType==='server'?' ☁️':''}`} color="#22C55E" small />
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:4, flexShrink:0 }}>
                    ...
                    {canUploadFile && (
                      <label style={{ background:"#3B82F622", border:"1px solid #3B82F644", borderRadius:6, padding:"4px 10px", color:"#3B82F6", cursor:"pointer", fontSize:10, fontWeight:700, textAlign:"center" }}>
                        ⬆ {(doc.dataUrl||doc.serverUrl) ? "Remplacer" : "Téléverser"}
                        {/* FIX v152 — Upload serveur via gcFileSave (remplace le dataUrl seul) */}
                        <input type="file" accept=".pdf,.docx,.doc,.xlsx,.xls,.jpg,.jpeg,.png,.pptx,.txt" style={{ display:"none" }} onChange={async e => {
                          const file = e.target.files?.[0]; if (!file) return;
                          try {
                            const ref = await gcFileSave(file, { module: 'processus-internal', uploadedBy: localUser.id, nom: file.name, taille: file.size, type: file.type });
                            setInternalDocs(prev => prev.map(d => d.id === doc.id ? {
                              ...d,
                              dataUrl: ref?.serverUrl ? null : (d.dataUrl || null),
                              serverUrl: ref?.serverUrl || null,
                              serverId: ref?.serverId || null,
                              storageType: ref?.serverUrl ? 'server' : 'local',
                              fileName: ref?.nom || file.name,
                              fileSize: file.size, sizeStr: gcFmtSize(file.size),
                              fileType: file.name.split('.').pop()?.toUpperCase() || d.fileType,
                              hasFile: true, uploadedBy: localUser.id, uploadedAt: new Date().toISOString()
                            } : d));
                            gcAlert(`✅ Fichier "${file.name}" téléversé sur "${doc.name}".`);
