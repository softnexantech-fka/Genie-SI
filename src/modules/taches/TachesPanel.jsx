        ...
        <div>
          ...
          {selectedTaches.length > 0 && (
            <div style={{background:"#3B82F611",border:"1px solid #3B82F644",borderRadius:8,padding:"8px 12px",marginBottom:8,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              ...
              <button onClick={async () => {if(await gcConfirm(`Marquer ${selectedTaches.length} tâche(s) comme terminées ?`)){setTaches&&setTaches(prev=>{const updated=prev.map(t=>selectedTaches.includes(t.id)?{...t,status:"TERMINE"}:t);dsSave('taches',updated);return updated;});setSelectedTaches([]);}}}
              ...
              <button onClick={async () => {if(await gcConfirm(`Supprimer ${selectedTaches.length} tâche(s) ?`)){setTaches&&setTaches(prev=>{const updated=prev.filter(t=>!selectedTaches.includes(t.id));dsSave('taches',updated);return updated;});setSelectedTaches([]);}}}
      ...
      {mainTab === "alertes_securite" && canSeeSessions && (
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            ...
            <button onClick={async () => {if(await gcConfirm("Effacer toutes les alertes sécurité ?"))try{_lsSet("gc-security-alerts","[]");setSiAlerts([]);dsSave("gc-security-alerts",[]).catch(err => gcToast.syncError('', err));}catch (_) {};}} style={{background:"#EF444415",border:"1px solid #EF444433",color:"#EF4444",borderRadius:8,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:700}}>🗑️ Purger</button>
          </div>
            ...
            {siAlerts.map((a,i)=>{
              ...
              return (
                <div key={a.id||i} style={{background:tc+"11",border:`1px solid ${tc}44`,borderRadius:10,padding:"10px 14px",display:"flex",gap:10,alignItems:"center"}}>
                  ...
                  <button onClick={()=>{const updated=siAlerts.filter((_,j)=>j!==i);setSiAlerts(updated);try{_lsSet("gc-security-alerts",JSON.stringify(updated));dsSave("gc-security-alerts",updated).catch(err => gcToast.syncError('', err));}catch (_) {};}} style={{background:"#EF444415",border:"1px solid #EF444433",color:"#EF4444",borderRadius:5,padding:"2px 7px",cursor:"pointer",fontSize:9}}>🗑️</button>
                </div>
              );
            })}
          </div>
