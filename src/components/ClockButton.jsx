...
// ClockButton.jsx — Horloge / Minuteur / Chrono / Alarmes
//
// [RÉORGANISATION — demande utilisateur] L'horloge est désormais intégrée dans la Boîte à
// Outils (ToolsWidget) avec les autres éléments (Calc Pro, Budget Rapide, Mémos…) au lieu
// d'être un bouton séparé dans la barre supérieure. Pour cela, ce fichier est réorganisé en
// trois parties réutilisables :
//   1. useClockAlarms() — état + effets des ALARMES uniquement (horloge qui tique, vérification
//      et déclenchement des alarmes, son, notification). [FIX P2-03 préservé] Cette logique doit
//      tourner en continu, INDÉPENDAMMENT du fait que le panneau visuel soit affiché ou non —
//      sinon une alarme programmée ne sonnerait que si l'utilisateur a la Boîte à Outils ouverte
//      sur l'onglet Horloge au moment précis du déclenchement, ce qui annulerait le correctif.
//   2. ClockAlarmWatcher — petit composant invisible (ne rend rien), monté en permanence dans
//      SIApp.jsx, qui utilise useClockAlarms() uniquement pour le déclenchement en arrière-plan
//      (son + notification), même quand personne ne regarde la Boîte à Outils.
//   3. ClockWidgetPanel — le contenu visuel complet (onglets Horloge/Chrono/Minuteur/Alarmes),
//      conçu pour être inséré directement DANS le panneau déjà existant de ToolsWidget (pas de
//      bouton déclencheur ni de positionnement absolu propres — le conteneur appelant les fournit).
//
// ClockButton (export conservé, non utilisé actuellement) reste disponible si un bouton
// horloge autonome dans la barre supérieure est de nouveau souhaité un jour.

// ── 1. Hook partagé : alarmes (tourne en continu, indépendamment de l'affichage) ────────────
export function useClockAlarms({ setNotifications = () => {} } = {}) {
  ...
  // Sync: alarmes créées depuis AgendaModule ou un autre poste / un autre composant horloge
  useRemoteSync({ 'gc-widget-alarms': setWAlarms });

  // Update horloge (1x/s — nécessaire pour la vérification des alarmes à la minute/seconde)
  useEffect(() => {
  ...
  // [FIX P2-03] Vérification des alarmes — tourne en permanence, peu importe si un panneau
  // visuel (ClockWidgetPanel) est monté ou non. La dépendance est juste [clockNow].
  useEffect(() => {
  ...
  return { clockNow, wAlarms, wSaveAlarms, wAlarmRinging, setWAlarmRinging, handleAddAlarm, handleDeleteAlarm };
}

// ── 2. Composant invisible — déclenche les alarmes en arrière-plan, à monter une seule fois ──
export function ClockAlarmWatcher({ setNotifications = () => {} }) {
  useClockAlarms({ setNotifications });
  return null;
}

// ── 3. Panneau visuel complet — à insérer dans le conteneur de ToolsWidget ──────────────────
export function ClockWidgetPanel({ T, setNotifications = () => {} }) {
  const { clockNow, wAlarms, wSaveAlarms, wAlarmRinging, setWAlarmRinging, handleAddAlarm, handleDeleteAlarm } = useClockAlarms({ setNotifications });
  ...
  // Basculer automatiquement sur l'onglet Alarmes quand une alarme sonne pendant que le
  // panneau est affiché (l'utilisateur regarde déjà la Boîte à Outils à ce moment-là).
  useEffect(() => {
    if (wAlarmRinging) setClockTab("alarmes");
  }, [wAlarmRinging]);
  ...
  const onAddAlarm = () => {
    ...
    handleAddAlarm({ time: wAlarmTime, label: wAlarmNewLabel, repeat: wAlarmRepeat });
    setWAlarmNewLabel("");
    const n = new Date();
  ...
  return (
    <div>
      {/* Tabs */}
      <div style={{ display: "flex", gap: 2, marginBottom: 10, borderBottom: `1px solid ${T.border}`, overflowX: "auto" }}>
        {[
      ...
      {clockTab === "alarmes" && (
        <div>
          ...
          <div style={{ background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 10, padding: "10px 12px", marginBottom: 10 }}>
            ...
            <button onClick={onAddAlarm} style={{ width: "100%", background: "#C41E3A", border: "none", color: "#fff", borderRadius: 8, padding: "8px", cursor: "pointer", fontWeight: 700, fontSize: 12 }}>
              + Ajouter alarme
            </button>
          </div>
          {/* Liste des alarmes */}
...
// ── Composant standalone (conservé pour compatibilité — non utilisé actuellement) ──────────
// Si un bouton horloge indépendant dans la barre supérieure est de nouveau souhaité, il suffit
// de le ré-importer et de le rendre comme avant ; toute la logique reste à jour automatiquement
// puisqu'elle est partagée avec le panneau intégré à la Boîte à Outils.
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button
        ...
        style={{ background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: T.textMuted, fontSize: 13, display: "flex", alignItems: "center", gap: 6, fontWeight: 700 }}
      >
        <span>🕐</span>
      </button>
      {open && (
        <div
          ...
          style={{ position: "absolute", right: 0, top: 42, width: 520, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, boxShadow: "0 16px 48px #0009", zIndex: 700, maxHeight: "90vh", overflowY: "auto", padding: 12 }}
        >
          <ClockWidgetPanel T={T} setNotifications={setNotifications} />
        </div>
      )}
    </div>
  );
