#!/usr/bin/env node
/**
 * RESET COMPLET - DONNÉES UNIQUEMENT
...
const JWT_TOKEN = 'admin-reset-token'; // Token admin
...
async function main() {
  ...
  log('Vérifiez le backup: genie_si_backup_*.db', 'warn');
  log('Les comptes utilisateurs SERONT PRÉSERVÉS', 'warn');
  
  // 3. Effacer chaque clé
  ...
  try {
    ...
    if (usersCount === 0 && gcUsersCount === 0) {
      log('⚠️  ATTENTION: Aucun compte trouvé! Vérifier la restauration.', 'warn');
    }
  } catch (e) {
    log(`⚠️  Impossible vérifier comptes: ${e.message}`, 'warn');
  }

  // 5. Vérifier wipe registry
  log('\nVérification wipe registry...', 'info');
  try {
    const wipeResp = await apiCall('GET', '/api/data/gc-wipe-registry');
    const wipeCount = wipeResp.data ? Object.keys(wipeResp.data).length : 0;
    log(`✅ Wipe registry actif: ${wipeCount} clés marquées`, 'success');
  } catch (e) {
    log(`⚠️  Wipe registry: ${e.message}`, 'warn');
  }
